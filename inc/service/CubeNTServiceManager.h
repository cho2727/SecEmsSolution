...
class WELLS_Export CubeNTServiceManager
{
public:
	CubeNTServiceManager( LPTSTR szServiceName, LPTSTR szDisplay );
	~CubeNTServiceManager(void);

	BOOL Install(	DWORD dwType		= SERVICE_WIN32_OWN_PROCESS,
...
public:
	...
	BOOL Start(LPCTSTR lpServiceArgs = NULL);
