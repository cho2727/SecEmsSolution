...
namespace SecProcMana
{
	...
	int				SecProcManager::AddProgram(const char* proc_name, int copy_no, bool auto_run, const char* config_file/* = NULL*/)
	{
		SecProcess *proc = GetProgram(proc_name, copy_no);
		if(proc != NULL)
		{
			return SEC_DATA_EXIST;
		}

		proc = new SecProcess(proc_name, copy_no, auto_run, config_file);
		proc_list_.push_back(proc);

		return SEC_OK;
	}

	SecProcess*		SecProcManager::GetProgram(const char* proc_name, ushort copy_no)
	{
